...
| Lesson | Detail |
|--------|--------|
| Track index varies | Timeline may have only 1 audio track → CLI creates "SFX 1" at index 2 automatically. Always check `probe_audio_track` before placing. |
...
| Transcript timecodes > text | Frame-based timecodes from `get_transcript` are more accurate for beat timing than raw subtitle text. |
...
| Library paths | Processed at `SFX_processed/`, raw at `SFX/`. Use processed for normalized levels. |
