...
Two systems convert beat analysis into executable SFX placement plans: the standalone script (`generate_sfx_plan.py`) and the engine recommender (`SFXRecommender`).

## Script Path (generate_sfx_plan.py)

### Input

Beats JSON from `analyze_subtitles.py` — list of dicts with `start_seconds`, `beat_type`, `sfx_suggestion`, `priority`, `text`.
...
4. Density cap: max = (duration / 60) × density_per_minute + 1
```
...
2. Extract events from subtitles/transcript
...
- Record frame: `int(round(beat.timestamp * fps))`
...
See also: [[beat-detection]], [[analysis-pipeline]], [[family-mapping]], [[negative-knowledge]]
